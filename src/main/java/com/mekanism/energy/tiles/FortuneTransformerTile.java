...
public class FortuneTransformerTile extends AbstractMechanism implements Container {
    ...
    public void onUpdateServer() {
        ...
        if (pickaxe.isEnchanted()) {
            for (Map.Entry<Enchantment, Integer> entry : pickaxe.getAllEnchantments().entrySet()) {
                if (entry.getKey().equals(Enchantments.BLOCK_EFFICIENCY)) {
                    defaultBreakTicks = defaultBreakTicks * (1 / entry.getValue());
                } else if (entry.getKey().equals(Enchantments.BLOCK_FORTUNE)) {
    ...
    private List<ItemStack> smeltAll(List<ItemStack> toSmelt) {
