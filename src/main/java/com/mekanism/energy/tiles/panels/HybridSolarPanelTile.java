...
public class HybridSolarPanelTile extends AbstractSolarPanel {
    ...
    public HybridSolarPanelTile(BlockPos pos, BlockState state) {
        super(BlockRegister.HYBRID_SOLAR_PANEL, pos, state, MekaEnergy.getSolar().hybridSolarPanel.get().multiply(2L), FloatingLong.create(100));
    }

    @NotNull
    @Override
    @SuppressWarnings(value = "unchecked")
    public ContainerTypeRegistryObject<MekanismTileContainer<HybridSolarPanelTile>> getContainerType() {
        return SolarContainerTypes.HYBRID_SOLAR_PANEL;
    }

    @Override
    protected FloatingLong getConfiguredMax() {
        return MekaEnergy.getSolar().hybridSolarPanel.get();
    }
}
